...
async def add_balance(message: Message, bot: Bot):
    """Пополнить баланс пользователя: /addbalance_<user_id>_<сумма>"""
    if not is_admin(message.from_user.id): return
    try:
    ...
    await message.answer(
        f"✅ Пользователю <b>{user['nickname'] if user else uid}</b> (ID: {uid})\n"
        f"Зачислено: <b>+{amount:.0f} ₽</b>\n"
        f"Баланс теперь: <b>{balance:.0f} ₽</b>",
        parse_mode="HTML"
    )
    try:
...
# ─── ПОПОЛНЕНИЕ СВОЕГО БАЛАНСА (для тестирования) ────────

@router.message(F.text.startswith("/mybalance_"))
async def my_balance_topup(message: Message):
    """Пополнить свой баланс для тестов: /mybalance_<сумма>"""
    if not is_admin(message.from_user.id): return
    try:
        amount = float(message.text.split("_")[1])
    except (IndexError, ValueError):
        await message.answer("❌ Формат: /mybalance_<сумма>")
        return
    await db.change_balance(message.from_user.id, amount)
    balance = await db.get_balance(message.from_user.id)
    await message.answer(
        f"✅ Твой баланс пополнен на <b>{amount:.0f} ₽</b>\n"
...
async def admin_panel(message: Message):
    ...
    lines.append("/mybalance_&lt;сумма&gt; — пополнить свой баланс (тест)")
    lines.append("/addbalance_&lt;user_id&gt;_&lt;сумма&gt; — пополнить баланс юзера")
    lines.append("/stats — статистика")
    lines.append("/admin — эта панель\n")
    if pending:
        lines.append(f"💸 <b>Ожидают вывода ({len(pending)}):</b>")
        for w in pending:
            lines.append(
                f"#{w['id']} | {w['nickname'] or w['username']} (ID:{w['user_id']}) | {w['amount']:.0f} ₽\n"
                f"→ /withdraw_done_{w['id']}"
            )
    else:
        lines.append("✅ Нет ожидающих выводов")
...
async def stats(message: Message):
    ...
    await message.answer(
        ...
        f"✅ Завершённых заказов: {orders}\n"
        f"💰 Заработано комиссии: {earned:.0f} ₽\n"
        f"💳 Всего пополнений: {topups:.0f} ₽",
        parse_mode="HTML"
    )
