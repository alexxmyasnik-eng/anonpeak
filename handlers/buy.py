...
async def check_payment(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if result is True:
        ...
        # Токен не настроен — ручная проверка
        from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="◀️ В меню", callback_data="main_menu")]
        ])
        await call.message.edit_text(
            f"⏳ Заявка #{order_id} отправлена на ручную проверку.\n"
            f"Администратор подтвердит оплату в течение 1 часа.",
            reply_markup=kb
        )
        await bot.send_message(
            ADMIN_ID,
            f"💳 Ручная проверка оплаты!\n"
            f"Заказ #{order_id} | {call.from_user.id}\n"
            f"Сумма: {order['amount']:.0f} ₽\n"
            f"Комментарий: {order['da_comment']}\n\n"
            f"Подтвердить: /confirm_order_{order_id}\n"
            f"Отклонить: /cancel_order_{order_id}"
        )
        await state.clear()
