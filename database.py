...
async def get_all_users():
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT * FROM users WHERE nickname IS NOT NULL") as c:
...
async def create_order(buyer_id, seller_id, product_id, amount, commission):
    async with aiosqlite.connect(DB_PATH) as db:
        c = await db.execute(
            "INSERT INTO orders (buyer_id,seller_id,product_id,amount,commission) VALUES (?,?,?,?,?)",
...
async def get_pending_orders_for_seller(seller_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT * FROM orders WHERE seller_id=? AND status='pending'", (seller_id,)
        ) as c:
            return await c.fetchall()
...
async def complete_withdrawal(withdrawal_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE withdrawals SET status='done' WHERE id=?", (withdrawal_id,))
        await db.commit()
...
async def get_dialog(user_a: int, user_b: int, limit: int = 30):
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
...
async def get_my_dialogs(user_id: int):
    """Возвращает список уникальных собеседников"""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            """SELECT DISTINCT
